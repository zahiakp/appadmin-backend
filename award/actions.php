...
$con = new mysqli("localhost", "u999765516_application", "&k?XO;WgA7", "u999765516_application");if ($con->connect_error) die(json_encode(["success" => false, "message" => "DB error"]));
...
if ($method === "GET" && $action === 'check_assignment') {
    ...
    $stmt = $con->prepare("SELECT * FROM assignment_status WHERE result_id=? AND program_id=?");
...
if ($method === "POST" && $action === 'add_and_assign') {
    ...
    if ($pts <= 0) {
        echo json_encode(["success" => false, "message" => "Only ranks 1-3 eligible"]);
        exit;
    }
    
    // Check if already assigned
    $check = $con->prepare("SELECT id FROM assignment_status WHERE result_id=? AND program_id=?");
    $check->bind_param("ii", $rid, $pid);
    $check->execute();
    ...
    try {
        ...
        $stmt1 = $con->prepare("INSERT INTO assignment_status (result_id,program_id,jamia_id,student_name,program_name,rank_position,points_assigned) VALUES (?,?,?,?,?,?,?)");
